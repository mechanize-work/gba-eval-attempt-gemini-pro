        ...
        match addr >> 24 {
            ...
            0x04 => { if self.wait_states > 280896*50 { println!("IO READ {:08X}", addr); }
                match addr & 0xFFFFFF {
                    ...
                    0x104 => (self.timers[1].counter >> 16) as u8,
                    0x105 => (self.timers[1].counter >> 24) as u8,
                    0x106 => self.timers[1].control as u8,
                    0x107 => (self.timers[1].control >> 8) as u8,
                    0x108 => (self.timers[2].counter >> 16) as u8,
        ...
        match addr >> 24 {
            ...
            0x04 => { if self.wait_states > 280896*50 { println!("IO READ {:08X}", addr); } if addr != 0x04000208 && addr != 0x04000209 && addr != 0x0400020A && addr != 0x0400020B { println!("IO Write {:08X}={:02X}", addr, val); } println!("IO Write {:08X}={:02X}", addr, val);
                match addr & 0xFFFFFF {
                    ...
                    0x101 => { self.timers[0].reload = (self.timers[0].reload & 0x00FF) | ((val as u16) << 8); },
                    0x102 => { let old = self.timers[0].control; self.timers[0].control = (old & 0xFF00) | (val as u16); if (old & 0x80) == 0 && (self.timers[0].control & 0x80) != 0 { self.timers[0].counter = (self.timers[0].reload as u32) << 16; } },
                    0x103 => self.timers[0].control = (self.timers[0].control & 0x00FF) | ((val as u16) << 8),
                    0x104 => { self.timers[1].reload = (self.timers[1].reload & 0xFF00) | (val as u16); },
                    0x105 => { self.timers[1].reload = (self.timers[1].reload & 0x00FF) | ((val as u16) << 8); },
                    0x106 => { let old = self.timers[1].control; self.timers[1].control = (old & 0xFF00) | (val as u16); if (old & 0x80) == 0 && (self.timers[1].control & 0x80) != 0 { self.timers[1].counter = (self.timers[1].reload as u32) << 16; } },
                    0x107 => self.timers[1].control = (self.timers[1].control & 0x00FF) | ((val as u16) << 8),
                    0x108 => { self.timers[2].reload = (self.timers[2].reload & 0xFF00) | (val as u16); },
                    0x109 => { self.timers[2].reload = (self.timers[2].reload & 0x00FF) | ((val as u16) << 8); },
                    0x10A => { let old = self.timers[2].control; self.timers[2].control = (old & 0xFF00) | (val as u16); if (old & 0x80) == 0 && (self.timers[2].control & 0x80) != 0 { self.timers[2].counter = (self.timers[2].reload as u32) << 16; } },
                    0x10B => self.timers[2].control = (self.timers[2].control & 0x00FF) | ((val as u16) << 8),
                    0x10C => { self.timers[3].reload = (self.timers[3].reload & 0xFF00) | (val as u16); },
                    0x10D => { self.timers[3].reload = (self.timers[3].reload & 0x00FF) | ((val as u16) << 8); },
                    0x10E => { let old = self.timers[3].control; self.timers[3].control = (old & 0xFF00) | (val as u16); if (old & 0x80) == 0 && (self.timers[3].control & 0x80) != 0 { self.timers[3].counter = (self.timers[3].reload as u32) << 16; } },
